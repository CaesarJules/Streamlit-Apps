...
filepath = os.path.join(path, 'cache_final.p')
...
st.sidebar.markdown(f"""<p style="font-size:22px;">
                        ...
                        <span style="color: #FF0000 ;text-align: center ;font-weight:bold; ">{latest_ww["confirmed"]:,}</span><br>
                        Total Deaths :<br>
                        <span style="color: #B8860B ; text-align: center ;font-weight:bold; ">{latest_ww["deaths"]:,}</span> <br>
...
graph_type = st.sidebar.selectbox('Cases type',('confirmed','deaths','new_cases'))
st.sidebar.subheader('Search by country or region 📍')
countries_selected = st.sidebar.multiselect('Country', regions)
if graph_type!='new_cases':
    num_weeks_selected = st.sidebar.slider('Number of weeks to display', 1, 5, 3)

#Plot the world heat map
#Use the selected graph_type

st.markdown(f"""<h4 style='text-align: justified;'>Number of coronavirus (COVID-19) cases worldwide on {latest_cache_date.strftime('%A %B %d, %Y')}</h4>""", unsafe_allow_html=True)
...
map_subtitle = "COVID-19 new cases worldwide"
if graph_type!='new_cases':
...
if graph_type=='new_cases':
    if len(countries_selected)>0:
        ...
        fig2_title = f"""<h4 style='text-align: justified;'>Number of COVID-19 new cases  in {", ".join(countries_selected)} as of {latest_cache_date.strftime('%A %B %d, %Y')}</h4>"""
        ...
        fig2_title = f"""<h4 style='text-align: justified;'>Top 5 countries with the highest number of COVID-19 new cases as of {latest_cache_date.strftime('%A %B %d, %Y')}</h4>"""
    ...
    if len(countries_selected)>0:
        ...
        fig2_title = f"""<h4 style='text-align: justified;'>Daily number of COVID-19 {graph_type} cases in {", ".join(countries_selected)} for the last {num_weeks_selected*7} days</h4>"""

        fig3_title = f"""<h4 style='text-align: justified;'>Daily number of COVID-19 {graph_type} cases in {", ".join(countries_selected)} since {datetime.datetime.strptime(data['dates'][0], "%Y-%m-%d").strftime('%A %B %d, %Y')}</h4>"""
        ...
        fig2_title = f"""<h4 style='text-align: justified;'>Daily number of COVID-19 {graph_type} cases worldwide over the last {num_weeks_selected*7} days</h4>"""
        ...
        fig3_title = f"""<h4 style='text-align: justified;'>Daily number of COVID-19 {graph_type} cases worldwide since {datetime.datetime.strptime(data['dates'][0], "%Y-%m-%d").strftime('%A %B %d, %Y')}</h4>"""
